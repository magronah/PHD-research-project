...
write.csv(nonchim, "Bimera_output_PRJNA644763.csv")
...
head(track)
write.csv(track, "read_track_PRJNA644763.csv")
...
write.csv(taxa_silva, "taxa_silva132_v3v4_PRJNA644763.csv")
saveRDS(taxa_silva, "taxa_silva132_v3v4_PRJNA644763.rds")

#unname(head(taxa_gg))
unname(head(taxa_silva))

#write to disk
write.csv(seqtab.nochim, "seqtab_nochim_v3v4_PRJNA644763.csv")
saveRDS(seqtab.nochim, "seqtab_nochim_v3v4_PRJNA644763.rds")
seqtab.nochim.trans = t(seqtab.nochim)
write.csv(seqtab.nochim.trans, "seqtab_nochim_transposed_v3v4_PRJNA644763.csv")
...
#met_d <- read.csv("meta_data.txt")
#aa <- read.csv("test.txt")
#bb <- which(met_d$AGE == "not collected")
#met_d <- met_d[-bb,]

#View(met_d)
#name <- substring(met_d$Sample.Name, 1,5)
#unique(name)
#l <- dim(met_d)[1]
...
#for (i in 1:l)
#{
#  rr <- c(rr, grep(list[i], readLines("test.txt"), value = TRUE))

#}
#write.csv(rr, file ="actual.csv", row.names = FALSE)
#nn<- read.csv("actual.txt")
#data<- data.frame(rr)
#length(unique(nn$x))
#data
