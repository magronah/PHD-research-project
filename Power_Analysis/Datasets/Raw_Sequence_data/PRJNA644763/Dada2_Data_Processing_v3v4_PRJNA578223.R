...
#define path
path = "~/Documents/Microbiome_data/PRJNA644763"  
...
# Read in OTU table
otu_df = read.csv("seqtab_nochim_transposed_v3v4_PRJNA644763.csv", row.names = 1) 
seqs = rownames(otu_df)
rownames(otu_df) = NULL
#View(otu_df) 

#Read in the taxfile 
tax_df = read.csv("taxa_silva132_v3v4_PRJNA644763.csv", row.names = 1)
...
met_df <- read.csv("meta_data.txt")
bb <- which(met_df$AGE == "not collected")
met_df <- met_df[-bb,]
#View(met_df) 
#dim(met_df)

#Detecting control from treatment groups
name <- substring(met_df$Sample.Name, 1,5)
unique(name)
...
write.csv(seqs, file="ASV_sequences_PRJNA644763.csv")
...
write.csv(samplesums, file="readcounts_per_sample_sorted_v3v4PRJNA644763.csv")
plot(samplesums)


#filter table of non-bacterial taxa
dat_lessHOST = subset_taxa(dat, Kingdom=="Bacteria", Family!="Mitochondria")
write.csv(otu_table(dat_lessHOST),file='seqtab_nochim_lessHOST_probgut_v3v4PRJNA644763.csv')
write.csv(tax_table(dat_lessHOST),file='taxa_JJML_lessHOST_v3v4PRJNA644763.csv')
...
write.csv(otu_table(rel_abun_all_prune),file='relative_abundance_v3v4PRJNA644763.csv')
rel_abun_table <- read.csv("relative_abundance_v3v4PRJNA644763.csv")
#View(rel_abun_table)
...
write.csv(otu_table(dat_r),file="rarefy_otu_table_v3v4PRJNA644763.csv")


kk <- data.frame(met_df$Run, substring(met_df$Sample.Name, 1,5)) 
...
#rr<-read.csv("seqtab_nochim_lessHOST_probgut_v3v4PRJNA644763.csv", row.names = 1)
#View(rr)
#stopifnot(colnames(rr) == kk$Samples)
