...
#define path
path = "~/Documents/Microbiome_data/PRJNA642975" 
...
# Read in OTU table
otu_df = read.csv("seqtab_nochim_transposed_v4_PRJNA642975.csv", row.names = 1) 
#View(otu_df)
...
#Read in the taxfile 
tax_df = read.csv("taxa_silva132_v4_PRJNA642975.csv", row.names = 1)
...
met_df = read.csv("metadata_PRJNA642975.txt") 
View(met_df) 
stopifnot(all(met_df$Run == colnames(otu_df)))
dim(met_df)

#Detecting control from treatment groups
Groups <- met_df$host_genotype
kr <- data.frame(met_df$Run, Groups)
colnames(kr)[1] <- "Sample"
unique(Groups)

write.csv(kr,file= "groups_names_PRJNA642975.csv", row.names= FALSE)
...
write.csv(seqs, file="ASV_sequences_PRJNA642975.csv")
...
write.csv(samplesums, file="readcounts_per_sample_sorted_v4_PRJNA642975.csv")
plot(samplesums)

#filter table of non-bacterial taxa
dat_lessHOST = subset_taxa(dat, Kingdom=="Bacteria", Family!="Mitochondria")
write.csv(otu_table(dat_lessHOST),file='seqtab_nochim_lessHOST_probgut_v4_PRJNA642975.csv')
write.csv(tax_table(dat_lessHOST),file='taxa_JJML_lessHOST_v4_PRJNA642975.csv')
...
write.csv(otu_table(rel_abun_all_prune),file='relative_abundance_v4_PRJNA642975.csv')
rel_abun_table <- read.csv("relative_abundance_v4_PRJNA642975.csv")
#View(rel_abun_table)
...
write.csv(otu_table(dat_r),file="rarefy_otu_table_v4_PRJNA642975.csv")
