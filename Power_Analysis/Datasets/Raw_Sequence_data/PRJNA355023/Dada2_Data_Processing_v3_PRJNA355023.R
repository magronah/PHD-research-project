...
#define path
path = "~/Documents/Microbiome_data/PRJNA355023" 
...
# Read in OTU table
otu_df = read.csv("seqtab_nochim_transposed_v3_PRJNA355023.csv", row.names = 1) 
seqs = rownames(otu_df)
rownames(otu_df) = NULL
#View(otu_df) 

#Read in the taxfile 
tax_df = read.csv("taxa_silva132_v3_PRJNA355023.csv", row.names = 1)
...
met_df = read.csv("metadata_PRJNA355023.txt") 
View(met_df) 
#dim(met_df)

#Detecting control from treatment groups
groups <- met_df$Host_disease
unique(groups)

#Check that the group names matches with what is in the metadata 
stopifnot(all(met_df$Run == colnames(otu_df)))

write.csv(groups,file="groups_PRJNA355023.csv")
...
write.csv(seqs, file="ASV_sequences_PRJNA355023.csv")
...
write.csv(samplesums, file="readcounts_per_sample_sorted_v3_PRJNA355023.csv")
plot(samplesums)


#filter table of non-bacterial taxa
dat_lessHOST = subset_taxa(dat, Kingdom=="Bacteria", Family!="Mitochondria")
write.csv(otu_table(dat_lessHOST),file='seqtab_nochim_lessHOST_probgut_v3_PRJNA355023.csv')
write.csv(tax_table(dat_lessHOST),file='taxa_JJML_lessHOST_v3_PRJNA355023.csv')
...
write.csv(otu_table(rel_abun_all_prune),file='relative_abundance_v3_PRJNA355023.csv')
...
write.csv(otu_table(dat_r),file="rarefy_otu_table_v3_PRJNA355023.csv") 
