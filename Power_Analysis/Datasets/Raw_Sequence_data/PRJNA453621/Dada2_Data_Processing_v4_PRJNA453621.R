...
# Read in OTU table
otu_df = read.csv("seqtab_trans_v4_PRJNA453621.csv", row.names = 1) 
seqs = rownames(otu_df)
rownames(otu_df) = NULL
#View(otu_df) 

#Read in the taxfile 
tax_df = read.csv("taxa_PRJNA453621.csv", row.names = 1)
...
met_df = read.csv("metadata_PRJNA453621.txt") 
View(met_df) 
#dim(met_df)

#Detecting control from treatment groups
Groups <- met_df$diagnosis
unique(Groups)

#Check that the group names matches with what is in the metadata 
stopifnot(all(met_df$Run == colnames(otu_df)))
grp <- data.frame(colnames(otu_df),Groups)
colnames(grp)[1] <- "Samples"
#View(grp)
write.csv(grp,file="groups_PRJNA453621.csv")
...
write.csv(seqs, file="ASV_sequences_PRJNA453621.csv")
...
write.csv(samplesums, file="readcounts_per_sample_sorted_v3_PRJNA453621.csv")
plot(samplesums)


#filter table of non-bacterial taxa
dat_lessHOST = subset_taxa(dat, Kingdom=="Bacteria", Family!="Mitochondria")
write.csv(otu_table(dat_lessHOST),file='seqtab_nochim_lessHOST_v4_PRJNA453621.csv')
write.csv(tax_table(dat_lessHOST),file='taxa_JJML_lessHOST_v4_PRJNA453621.csv')
...
write.csv(otu_table(rel_abun_all_prune),file='relative_abundance_v4_PRJNA453621.csv')
...
write.csv(otu_table(dat_r),file="rarefy_otu_table_v4_PRJNA453621.csv") 
