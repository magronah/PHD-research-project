...
# Read in OTU table
otu_df = read.csv("seqtab_nochim_transposed_v3v4_PRJNA578223.csv", row.names = 1) 
seqs = rownames(otu_df)
rownames(otu_df) = NULL
#View(otu_df) 

#Read in the taxfile 
tax_df = read.csv("taxa_silva132_v3v4_PRJNA578223.csv", row.names = 1)
...
met_df = read.csv("meta_PRJNA578223.txt") 
#View(met_df) 
#dim(met_df)
...
write.csv(seqs, file="ASV_sequences_PRJNA578223.csv")
...
write.csv(samplesums, file="readcounts_per_sample_sorted_v3v4PRJNA578223.csv")
plot(samplesums)


#filter table of non-bacterial taxa
dat_lessHOST = subset_taxa(dat, Kingdom=="Bacteria", Family!="Mitochondria")
write.csv(otu_table(dat_lessHOST),file='seqtab_nochim_lessHOST_probgut_v3v4PRJNA578223.csv')
write.csv(tax_table(dat_lessHOST),file='taxa_JJML_lessHOST_v3v4PRJNA578223.csv')
...
write.csv(otu_table(rel_abun_all_prune),file='relative_abundance_v3v4PRJNA578223.csv')
rel_abun_table <- read.csv("relative_abundance_v3v4PRJNA578223.csv")
#View(rel_abun_table)
...
write.csv(otu_table(dat_r),file="rarefy_otu_table_v3v4PRJNA578223.csv")
