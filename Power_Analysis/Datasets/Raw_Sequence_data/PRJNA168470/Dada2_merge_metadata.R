...
# Read in OTU table
otu_df = read.csv("seqtab_trans_v2v3_PRJNA168470.csv", row.names = 1) 
seqs = rownames(otu_df)
rownames(otu_df) = NULL
 
#Read in the taxfile 
tax_df = read.csv("taxa_PRJNA168470.csv", row.names = 1)
#View(tax_df)
...
met_df = read.csv("metadata_PRJNA168470.txt") 
View(met_df) 
#dim(met_df)

#Extracting group information
gp <- met_df$Sample.Name
groups <- substring(gp,1,1)
unique(groups)
sum(groups == "N")
sum(groups == "A")
write.csv(groups,file= "groups_PRJNA168470.csv", row.names= FALSE)
...
write.csv(seqs, file="ASV_PRJNA168470_sequences.csv")
...
write.csv(samplesums, file="readcounts_per_sample_sorted_PRJNA168470.csv")
plot(samplesums)

#filter table of non-bacterial taxa
dat_lessHOST = subset_taxa(dat, Kingdom=="Bacteria", Family!="Mitochondria")
write.csv(otu_table(dat_lessHOST),file='seqtab_nochim_lessHOST_PRJNA168470.csv')
write.csv(tax_table(dat_lessHOST),file='taxa_JJML_lessHOST_PRJNA168470.csv')
...
write.csv(otu_table(rel_abun_all_prune),file='relative_abundance_PRJNA168470.csv')
rel_abun_PRJNA589343 <- read.csv("relative_abundance_PRJNA168470.csv")
...
write.csv(otu_table(dat_r),file="rarefy_otu_PRJNA168470.csv")
