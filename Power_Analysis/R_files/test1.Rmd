...
# m1 <- glmer(count ~group*time+(-1 + taxa|subject:time),
#             data = sim_data, family = negative.binomial(theta=1))
# 
# getME(m1.)
# gg <- simulate(m1,10)
# View(gg)
...
\begin{table*}[h]
  ...
  \caption{Parameter values used in data simulation}
    \begin{tabular}{l|cc}
          ...
          Intercept = 1 & Random effect parameter ($\theta$) = Identity \\
          Group = 0.2   & Number of Taxa = 10 \\
          Time  =0.1   & Length of Time  = 5\\
...
<!--     fit_glmmtmb_rr = glmmTMB(count ~ group*time+ rr(-1 + taxa|subject:time,3), -->
...
<!-- v = compare_models(nIndiv=20, nTaxa=100, nTime = 10, Beta=  c(1, 0.2, 0.1, 0.2),  nSim = 100) -->
<!-- ## Save these datasets -->
...
<!--       geom_line(aes(y = other),color = "black") + -->
<!--       geom_line(aes(y = glmmtmb), color = "red") + -->
